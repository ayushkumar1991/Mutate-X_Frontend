...
export default function KnownVariants({
  ...
  const analyzeVariant = async (variant: ClinvarVariant) => {
    ...
    // Safely access title with optional chaining
    ...
    if (
      ...
      // Handle cases where variant details can't be parsed
      updateClinvarVariant(variant.clinvar_id, {
        ...variant,
        isAnalyzing: false,
        evo2Error: "Could not parse variant details from title.",
      });
      return;
    }
  ...
  const exportToPDF = () => {
    ...
    const geneLabel = gene?.symbol || gene?.name || "Gene";
    const generatedOn = new Date();

    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(14);
    pdf.text(`Known Variants in ${geneLabel} (ClinVar)`, marginX, y);

    pdf.setFontSize(10);
    y += 18;
    pdf.text(`Genome: ${genomeId || "-"}`, marginX, y);
    y += 14;
    pdf.text(`Chromosome: ${gene?.chrom || "-"}`, marginX, y);
    y += 14;
    pdf.text(`Generated: ${generatedOn.toLocaleString()}`, marginX, y);
    ...
    const rows = clinvarVariants.map((v) => [
      v.title || "-",
      v.variation_type || "-",
      v.classification || "Unknown",
      v.location || "-",
      v.evo2Result?.prediction || "-",
    ]);
    ...
    autoTable(pdf, {
      ...
      didDrawPage: (data) => {
        ...
        pdf.setFontSize(9);
        pdf.text(
 // Use the more standard way to get page number
    ...
    const filename = `known_variants_${geneLabel}_${fileDate}.pdf`.replace(
      /\s+/g,
      "_"
    );
    pdf.save(filename);
      ...
      <CardHeader className="flex flex-row items-center justify-between pt-4 pb-2">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            size="sm"
            onClick={exportToPDF}
            disabled={isLoadingClinvar || clinvarVariants.length === 0}
                  ...
                  >
                    ...
                    <TableCell className="py-2 text-xs">
                      <div className="flex flex-col items-end gap-1">
                        {/* SAFELY check variation_type before calling methods on it */}
                        {variant.variation_type
                          ?.toLowerCase()
